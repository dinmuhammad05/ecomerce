...
import { EvaluateDto } from './dto/evaluate-student.dto';
...
export class StudentController {
  ...
  @Get('my-students')
  @ApiOperation({ summary: 'Get all students for teacher' })
  ...
  @Get('my-students/:id')
  @ApiOperation({ summary: 'Get my student by id' })
  @accessRoles(Roles.STUDENT)
  findMyStudent(
    ...
    return this.studentService.findOneById(id, {
      where: { group: { teacher: { id: user.id } } },
    });
  }
  ...
  @Get('details')
  @ApiOperation({
    summary: 'Get student by id for admin , superadmin and teacher',
  })
  ...
  @Patch('evaluate/:id')
  @ApiOperation({ summary: 'Update student for teachers' })
  @accessRoles(Roles.TEACHER)
  evaluate(
    @CurrentUser() user: IToken,
    @Param('id', ParseUUIDPipe) id: string,
    ...
    return this.studentService.evaluateGrade(user.id, id, evaluateDto);
  }
